...
class AudioDeviceMonitor(threading.Thread):
    ...
    def _poll_devices(self, initial=False):
        # IMPORTANT: We query PyAudio directly, NOT via subprocess.run([sys.executable, ...]).
        # In PyInstaller, sys.executable IS the compiled .exe, so spawning it would
        # launch a full new copy of Cortex — a fork-bomb that deadlocks ML libraries.
        import pyaudio
        import sys, os

        try:
            # Suppress ALSA/PortAudio warnings from C libraries
            devnull_fd = os.open(os.devnull, os.O_WRONLY)
            if hasattr(sys, 'stderr') and sys.stderr:
                pass  # Python-level stderr is fine
            
            p = pyaudio.PyAudio()
            try:
                api_index = p.get_default_host_api_info()['index']
            except Exception:
                api_index = None

            try:
                def_in = p.get_default_input_device_info().get('name')
            except Exception:
                def_in = None

            try:
                def_out = p.get_default_output_device_info().get('name')
            except Exception:
                def_out = None
            ...
            for i in range(p.get_device_count()):
                try:
                    ...
                    if api_index is not None and info.get('hostApi') != api_index:
                        continue
                    name = info.get('name', '')
                    if not name or 'Microsoft Sound Mapper' in name:
                        continue
            ...
            new_inputs = set(d for d in ins if not _is_virtual_linux_device(d))
            new_outputs = set(d for d in outs if not _is_virtual_linux_device(d))
            new_def_in = def_in
            new_def_out = def_out

            if not initial:
                changed_any = False
                if self._check_diff(self.current_inputs, new_inputs, "Microphone"):
                    changed_any = True
                if self._check_diff(self.current_outputs, new_outputs, "Speaker"):
                    changed_any = True
